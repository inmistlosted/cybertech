    ...
    public class OrderService : IOrderService
    {
        public async Task CreateOrder(Order order)
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    RequestUri = new Uri($"https://localhost:5012/api/order")
                };
        ...
        public async Task DeleteOrderById(Guid? id)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage()
                {
                    RequestUri = new Uri($"https://localhost:5012/api/order/{id}")
                };
        ...
        public async Task<List<Order>> GetAllOrders(string id)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage()
                {
                    RequestUri = new Uri($"https://localhost:5012/api/order/supplier/{id}")
                };
        ...
        public async Task UpdateOrder(Order order)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage()
                {
                    RequestUri = new Uri($"https://localhost:5012/api/order/{order.Id}")
                };
