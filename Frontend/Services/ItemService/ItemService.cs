    ...
    public class ItemService : IItemService
    {
        public async Task CreateItem(ShopItem item)
        {
            using(var client = new HttpClient())
            {
                ...
                {
                    RequestUri = new Uri($"https://localhost:5014/api/product")
                };
        ...
        public async Task DeleteItemById(Guid? id)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage
                {
                    RequestUri = new Uri($"https://localhost:5014/api/product/{id}")
                };
        ...
        public async Task UpdateItem(ShopItem item)
        {
            using (var client = new HttpClient())
            {
                var request = new HttpRequestMessage
                {
                    RequestUri = new Uri($"https://localhost:5014/api/product/{item.Id}")
                };
                var json = JsonConvert.SerializeObject(item);
